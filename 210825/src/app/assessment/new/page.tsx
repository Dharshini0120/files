...
import hospitalTemplate from "../../../data/hospitalTemplate.json";
...
const AssessmentPage = () => {
  ...
  // Load template from API response (stored in sessionStorage) or fallback to local file
  const loadTemplateFromFile = () => {
    try {
      // First try to get template from API response (stored in sessionStorage)
      const apiTemplateData = sessionStorage.getItem('selectedTemplate');

      if (apiTemplateData) {
        const apiTemplate = JSON.parse(apiTemplateData);
        console.log('🔄 Loading template from API:', apiTemplate);

        // Convert API response to Template format
        const convertedTemplate: Template = {
          id: apiTemplate.id,
          name: apiTemplate.name,
          description: `${apiTemplate.name} - Version ${apiTemplate.version}`,
          nodes: apiTemplate.questionnaire?.nodes || [],
          edges: apiTemplate.questionnaire?.edges || [],
          status: 'Completed',
        ...
        console.log('🔄 Template name:', convertedTemplate.name);
        console.log('🔄 Number of nodes:', convertedTemplate.nodes.length);

        const hasUsableTemplate =
          convertedTemplate.nodes &&
          Array.isArray(convertedTemplate.nodes) &&
          convertedTemplate.nodes.length > 0;
      ...
      console.log('🔄 Falling back to local template file');
      const tmpl = (hospitalTemplate as unknown) as Partial<Template>;
      const hasUsableTemplate =
        tmpl &&
        Array.isArray(tmpl.nodes) &&
        tmpl.nodes.length > 0 &&
        Array.isArray(tmpl.edges);

      if (!hasUsableTemplate) {
        alert("No valid template found. Please try creating a new assessment from the main page.");
        router.push("/assessment");
        return;
      }

      // Fill any missing meta fields so existing code remains happy
      const hydrated: Template = {
        id: (tmpl.id as string) ?? "imported-template",
        name: (tmpl.name as string) ?? "Imported Template",
        description: (tmpl.description as string) ?? "",
        nodes: tmpl.nodes as any[],
        edges: tmpl.edges as any[],
        status: (tmpl.status as string) ?? "Completed",
        createdAt: (tmpl.createdAt as string) ?? new Date().toISOString(),
        updatedAt: (tmpl.updatedAt as string) ?? new Date().toISOString(),
      };
  ...
  const getQuestionsForSection = (sectionId: string) => getReachableQuestions(sectionId);
          ...
          <Box sx={{ px: 0, pt: 1.5, pb: 1, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <Box>
              ...
              <Typography variant="subtitle1" color="#6c757d" fontFamily={"var(--font-inter), sans-serif"}>
                {template?.name || "Assessment Template"} - Started on{" "}
